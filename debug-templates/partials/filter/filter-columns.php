<?php
/**
 * Template: Filter Columns Layout (Spaltenansicht)
 *
 * Displays each filter group as a column with a labeled border.
 * Column widths follow these rules:
 *   - 1 group  → 100 %
 *   - 2 groups → 50 % each
 *   - 3 groups → 50 %, 50 %, 100 % (last wraps to its own row)
 *   - 4 groups → 50 % each
 *   - etc. (odd last group always 100 %)
 *
 * Available variables:
...
?>
<nav class="jpkpf-filter-columns<?php echo ! empty( $extra_class ) ? ' ' . esc_attr( $extra_class ) : ''; ?>"
    ...
    if ( ! empty( $filter_groups ) ) :
        foreach ( $filter_groups as $group ) :
            ...
            ?>
            <div class="jpkpf-filter-columns-group" data-filter-taxonomy="<?php echo esc_attr( $group['taxonomy'] ); ?>">
                <span class="jpkpf-filter-columns-label"><?php echo esc_html( $group['label'] ); ?></span>
